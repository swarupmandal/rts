...
public class OverdueMonthPdf {
	private String filePath;
	private Document document= null;
	private PdfWriter writer = null;
	ParagraphBorder border; 
	private String printDate = new SimpleDateFormat("dd-MMM-yyyy").format(new Date());
	private ArrayList<OverdueMonthBean> finalList = null;
	
	public void getDetails(String realPath,String name,ArrayList<OverdueMonthBean> list)throws Exception
	 {
	...
	public void createDetails(String name, ArrayList<OverdueMonthBean> list) throws Exception {
		 System.out.println("INSIDE METHOD list size::: "+list.size() );
	        PdfPTable table = new PdfPTable(2);
	        table.setSplitLate(false);
	        table.setWidths(new int[]{10, 30});
	        for (OverdueMonthBean overdueMonthBean : list) {
	        	cell_1: {
					PdfPCell cell;
				
					Paragraph headerParagraph = new Paragraph(overdueMonthBean.getMonth());
	            ...
	            table.addCell(createtable(overdueMonthBean.getYearList()));
	        }
	        document.add(table);
	 }
