					...
					sql_insert:{
					    ...
					    try {
					    	preparedStatementInsert = Pstm.createQuery(connection, 
									RoleMasterSql.insertRoleQuery, Arrays.asList(roleMasterBean.getRoll().trim(),roleMasterBean.getUserid()));
		...
		try {
			...
			sql_connection:{
				try {
					...
					if( isSavedAssign){
						Messagebox.show(" Approver assignment Saved successfully.","Information",Messagebox.OK,Messagebox.INFORMATION);
					}else{
			...
			sql_connection:{
				try {
					...
					sql_insert:{
					    ...
					    try {
					    	preparedStatementInsert = Pstm.createQuery(connection, 
									RoleMasterSql.updateRoleQuery, Arrays.asList(roleMasterBean.getRoll().trim(),roleMasterBean.getRollId()));
					...
					sql_fetch:{
					   ...
					   try {
						   ...
						   while (resultSet.next()) {
								...
								bean.getUserprofileBean().setUserid(resultSet.getString("user_id"));
								...
								bean.getUserprofileBean().setId(resultSet.getInt("id"));
			...
			sql_connection:{
				try {
					...
					sql_insert:{
					    ...
					    try {
					    	preparedStatementInsert = Pstm.createQuery(connection, 
																RoleMasterSql.updateMappingQuery, Arrays.asList(roleMasterBean.getDownBean().getRollId(),
											                                                                    roleMasterBean.getMapperId()));
					...
					if( isUpdated){
						Messagebox.show(" Assign Role Details Updated successfully. ","Information",Messagebox.OK,Messagebox.INFORMATION);
					}else{
						Messagebox.show(" Assign Role Details failed due to internal error!","ERROR",Messagebox.OK,Messagebox.ERROR);
					}
				} catch (Exception e) {
					e.printStackTrace();
					logger.error(e);
